...
def lambda_handler(event, context):
    ...
    # Extract album data from the event
    albums = event.get('albums', [])
    
    # Log the received data
    print('Received albums data:', albums)
    
    # Handle missing or empty albums data
    ...
    <body>
        <div class="container">
            <div class="header">
                <h2>Top 10 Albums by Interaction</h2>
                <p>Generated Report</p>
            </div>
            <div class="content">
                <p>Hello,</p>
                <p>Below is a summary of the top 10 albums based on user interactions.</p>
                <table>
                    <thead>
                        <tr>
                            <th>Album Name</th>
    ...
    # Loop through the albums and add each one as a row in the table
    for album in albums:
        album_name = album.get('album_name', 'Unknown')
        total_interactions = album.get('total_interactions', 'N/A')
        body_html += f"""
                        <tr>
                            <td>{album_name}</td>
