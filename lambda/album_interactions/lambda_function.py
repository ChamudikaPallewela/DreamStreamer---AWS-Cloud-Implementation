...
def lambda_handler(event, context):
    ...
    try:
        ...
        album_id = body['albumId']  # Add albumId to request body
        album_name = body['albumName']  # Add albumName to request body
        ...
        create_table_query = """
        CREATE TABLE IF NOT EXISTS album_interactions (
        ...
        # Insert or update the album interaction
        insert_or_update_query = """
        INSERT INTO album_interactions (album_id, album_name, username, interaction_time, interaction_count)
        VALUES (%s, %s, %s, %s, %s)
        ON DUPLICATE KEY UPDATE interaction_count = interaction_count + 1, interaction_time = %s;
        """
        
        with connection.cursor() as cursor:
            cursor.execute(insert_or_update_query, (album_id, album_name, username, interaction_time, count, interaction_time))
